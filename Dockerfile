...
RUN mkdir /usr/local/bin/BAR
RUN chown -R aceuser:aceuser /usr/local/bin/BAR
COPY ./acesoe/binary/Microservice2.bar /usr/local/bin/BAR
...
RUN bash -c 'mqsibar -w /home/aceuser/ace-server -a /usr/local/bin/BAR/Microservice2.bar -c'
#USER aceuser
